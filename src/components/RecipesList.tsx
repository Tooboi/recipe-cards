  ...
  return (
    <div className="overflow-hidden">
      <div className="px-6 pt-5 rounded-none">
        <div className="text-xl text-center mx-auto text-slate-900 w-full font-semibold">Explore Recent Recipes</div>
      </div>
      <div className="pt-2">
            ...
            {initialRecipes.map((recipe) => (
              <Link href={`/recipes/${recipe.id}`} key={recipe.id} className="block col-span-1">
                <div className="px-4 py-2 border border-slate-600 rounded-lg bg-slate-300 hover:bg-slate-100 transition-colors group">
                  ...
                  <div className="text-sm text-slate-600">{recipe.user.username}</div>
                  <div className="text-xs text-slate-600">{formatDate(recipe.updatedAt)}</div>
